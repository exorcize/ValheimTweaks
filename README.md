...
**Store verification:** saving a chest stamps a revision on it, so the instant that
revision is replaced by data from the network is the only instant a store can be undone.
That is when the check looks, instead of on a timer. It reports and never recreates the
item: telling a network overwrite from the other player simply taking it is not possible
from one machine, and inventing an item on a good guess trades a loss you can read for a
duplicate you cannot see. What you get is the chest, the item and the amount in the log at
the moment it happens; recovery is the snapshot below, which works from a record.
